...
class teleop_control(Node):
    ...
    def save_pizza_positions(self):
        """Save pizza positions to a YAML file in the desired format."""
        self.get_logger().info("save_pizza_positions called")
        ...
        # Convert positions to plain dictionaries
        ...
        # Create new entry
        save_data = {
            'new_pizzas': pizza_positions_plain,
            'save_count': self.save_count + 1  # Increment save count in file
        }

        # Append new data to 'positions' list
        data['positions'].append(save_data)
            ...
            with open(self.yaml_file, 'r') as file:
                ...
                if 'positions' in data:
                    ...
                    for entry in data['positions']:
                        ...
                        self.pizza_positions.extend(new_pizzas)
                    self.save_count = len(data['positions'])
                    self.last_saved_pizza_count = len(self.pizza_positions)
                    self.get_logger().info(f"Loaded {len(self.pizza_positions)} pizza positions from file.")
                else:
                    self.get_logger().info("No 'positions' key found in YAML file.")
        else:
            self.get_logger().info("No pizza positions file found. Starting fresh.")

    def display_key_bindings(self):
        key_bindings = """
        KEY BINDINGS FOR TELEOP CONTROL
        -------------------------------
        'w' : Move forward
        's' : Move backward
        'a' : Turn left
        'd' : Turn right
        'j' : Spawn pizza at the current turtle position
        'k' : Save the positions of newly spawned pizzas to a file (only if auto-move is not active)
        'l' : Start moving to eat unsaved pizzas, which are the pizzas spawned since the last save
        """
        print(key_bindings)
    ...
    def start_eating_unsaved_pizzas(self):
        ...
        if not unsaved_pizzas:
            ...
            return
        self.target_positions = unsaved_pizzas.copy()  # Use copy to prevent modification during iteration
        self.current_target_index = 0
        self.auto_move = True
